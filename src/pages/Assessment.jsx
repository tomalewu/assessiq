...
// ── CV Upload (MTO only — Google Drive link) ─────────────────────────
function CVUpload({ candidate, role }) {
  ...
  const submit = async () => {
    if (!link.trim()) { setError('Please paste your Google Drive CV link'); return }
    if (!link.includes('drive.google.com') && !link.includes('docs.google.com') && !link.startsWith('http')) {
      setError('Please enter a valid link (Google Drive, Dropbox, or any URL)'); return
    }
    setSaving(true)
    try {
      const { dbSaveCandidate } = await import('../db')
  ...
  return (
    <div className="card card-xl au" style={{ padding:28, animationDelay:'.4s' }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:18 }}>
        <div style={{ width:40, height:40, borderRadius:11, background:'var(--ok-dim)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>📎</div>
        <div>
          <div style={{ fontWeight:800, fontSize:16 }}>Submit Your CV</div>
          <div style={{ fontSize:12, color:'var(--ink3)', marginTop:2 }}>You passed! Please share your CV to complete your application.</div>
        </div>
      </div>

      <div style={{ background:'var(--paper2)', borderRadius:10, padding:'14px 16px', marginBottom:16, fontSize:12, color:'var(--ink2)', lineHeight:1.7 }}>
        <strong>How to share your CV:</strong><br/>
        1. Upload your CV to Google Drive<br/>
        2. Right-click the file → Share → Change to "Anyone with the link"<br/>
        3. Copy the link and paste it below
      </div>

      <div className="fg" style={{ marginBottom:12 }}>
        <label className="fl">CV Link (Google Drive, Dropbox, or any file link)</label>
      ...
      <button className="btn btn-p btn-lg" style={{ width:'100%', justifyContent:'center' }}
        ...
        {saving
          ? <><span className="sp sp-sm" style={{ borderTopColor:'#fff' }}/> Submitting...</>
          : 'Submit CV & Complete Application'}
      </button>
      <p style={{ textAlign:'center', fontSize:11, color:'var(--ink3)', marginTop:10, lineHeight:1.6 }}>
        Your CV will only be shared with the hiring team for the <strong>{role.title}</strong> position.
...
// ── CV Upload (MTO only) ──────────────────────────────────────────────
function CVUpload({ candidate, role, result, profile }) {
  const [file, setFile]         = useState(null)
  const [uploading, setUploading] = useState(false)
  ...
  const handleUpload = async () => {
    if (!file && !driveLink.trim()) { setError('Please select a CV file or paste a Google Drive link'); return }
    setUploading(true)
    setError('')
    try {
      let cvUrl = driveLink.trim()
      ...
      if (file) {
        ...
        const app     = getApps().length ? getApp() : initializeApp(FB_CFG)
        const storage = getStorage(app)
        const path    = 'cvs/' + candidate.id + '_' + file.name.replace(/[^a-zA-Z0-9._-]/g, '_')
        const snap    = await uploadBytes(ref(storage, path), file)
        cvUrl         = await getDownloadURL(snap.ref)
      }

      // Save CV URL to candidate record
      const { dbSaveCandidate } = await import('../db')
      await dbSaveCandidate(candidate.id, { cvUrl, cvFileName: file ? file.name : driveLink, cvSubmittedAt: new Date().toISOString() })
  ...
  if (uploaded) return (
    <div style={{ background:'var(--ok-dim)', border:'1.5px solid var(--ok)', borderRadius:14, padding:'24px', textAlign:'center' }}>
      <div style={{ fontSize:36, marginBottom:12 }}>✅</div>
      <div style={{ fontWeight:800, fontSize:17, color:'var(--ok)', marginBottom:8 }}>CV Submitted Successfully!</div>
      <p style={{ fontSize:13, color:'var(--ink2)', lineHeight:1.7 }}>
        Your CV has been submitted for the <strong>{role.title}</strong> position.
        Our recruitment team will review your application and contact you if you are selected for the next stage.
      </p>
      <div style={{ marginTop:16, padding:'12px 16px', background:'var(--paper)', borderRadius:9, fontSize:12, color:'var(--ink3)' }}>
        A confirmation has been noted. Please keep an eye on your email at <strong>{candidate.email}</strong>.
      </div>
    </div>
  )
  ...
  return (
    <div className="card card-xl au" style={{ padding:28, animationDelay:'.4s' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
        <div style={{ width:36, height:36, borderRadius:10, background:'var(--ok-dim)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>📎</div>
        <div>
          <div style={{ fontWeight:800, fontSize:16 }}>Upload Your CV</div>
          <div style={{ fontSize:12, color:'var(--ink3)', marginTop:2 }}>Congratulations on passing! Please upload your CV to complete your application.</div>
        </div>
      </div>

      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        <div className="fg">
          <label className="fl">Upload CV File (PDF, DOC, DOCX)</label>
          <input type="file" accept=".pdf,.doc,.docx"
            onChange={e => { setFile(e.target.files[0]); setError('') }}
            style={{ padding:'10px', border:'1.5px dashed var(--line)', borderRadius:'var(--r-sm)', width:'100%', fontSize:13, cursor:'pointer', background:'var(--paper2)' }}/>
        </div>
        <div style={{ textAlign:'center', fontSize:12, color:'var(--ink3)' }}>— or —</div>
        <div className="fg">
          <label className="fl">Paste Google Drive Link</label>
          <input className="fi" type="url" placeholder="https://drive.google.com/file/..."
            value={driveLink} onChange={e => { setDriveLink(e.target.value); setError('') }}/>
        </div>
        {error && <span className="ferr">{error}</span>}
      </div>

      <button className="btn btn-p btn-lg" style={{ width:'100%', justifyContent:'center', marginTop:16 }}
        onClick={handleUpload} disabled={uploading}>
        {uploading
          ? <><span className="sp sp-sm" style={{ borderTopColor:'#fff' }}/> Uploading...</>
          : 'Submit CV & Complete Application'}
      </button>
      <p style={{ textAlign:'center', fontSize:11, color:'var(--ink3)', marginTop:10, lineHeight:1.6 }}>
        Your CV will be shared with the hiring team for the <strong>{role.title}</strong> position only.
      </p>
    </div>
  )
}
