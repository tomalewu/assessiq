    ...
    <div className="card" style={{ padding:0, overflow:'hidden', marginBottom:12 }}>
      ...
      <div style={{ padding:'16px 20px', cursor:'pointer', display:'flex', alignItems:'center', gap:12 }}
        ...
        <div style={{ display:'flex', gap:6, alignItems:'center', flexShrink:0 }}>
          ...
          {role.expiryDate && (userIsAdmin || role.createdBy===me?.id) && (
            <button className="btn btn-g btn-sm" style={{ fontSize:11,padding:'5px 8px',color:'var(--ok)' }}
              title="Extend expiry by 30 days"
              onClick={e=>{e.stopPropagation();const d=new Date(role.expiryDate);d.setDate(d.getDate()+30);onEditThreshold(role.id+'_expiry',d.toISOString().split('T')[0])}}>
              ⏰+30d
            </button>
          )}
          {userIsAdmin && (
