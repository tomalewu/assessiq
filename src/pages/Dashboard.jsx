...
function exportExcel(candidates, roles) {
  ...
  rows.forEach((row, i) => {
    const result = row[11]
    const rank   = row[0]
    let rowClass = result === 'Pass' ? 'pass' : result === 'Fail' ? 'fail' : ''
      ...
      <div style={{ padding:'16px 20px', cursor:'pointer', display:'flex', alignItems:'center', gap:12 }}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:'flex', gap:12, fontSize:12, color:'var(--ink3)', flexWrap:'wrap' }}>
            ...
            {role.expiryDate && (() => {
              const exp  = new Date(role.expiryDate)
              const now  = new Date()
              const days = Math.ceil((exp - now) / (1000*60*60*24))
              const expired = days < 0
              const soon    = days >= 0 && days <= 2
              return (
                <span style={{color: expired?'var(--bad)':soon?'var(--warn)':'var(--ink3)', fontWeight: expired||soon?700:400}}>
                  {expired ? 'Expired' : soon ? 'Expires in ' + days + 'd' : 'Expires ' + exp.toLocaleDateString('en-GB',{day:'numeric',month:'short'})}
                </span>
              )
            })()}
        ...
        <div style={{ display:'flex', gap:6, alignItems:'center', flexShrink:0 }}>
          ...
          {userCanEdit && (userIsAdmin || role.createdBy===me?.id) && (
            <button className="btn btn-g btn-sm" style={{ fontSize:11,padding:'5px 8px',color:'var(--ok)' }}
              ...
              {role.expiryDate ? '⏰ Edit Expiry' : '⏰ Set Expiry'}
            </button>
          )}
          {userIsAdmin && (
...
function CVInsightsTab({ candidates, roles, onViewResult }) {
  ...
  const sorted = [...filtered].sort((a,b) => {
    ...
    if (sortBy === 'exp') return ((b.cvParsed?.totalExperience)||0) - ((a.cvParsed?.totalExperience)||0)
    return 0
  })
    ...
    <div>
      <div className="shdr" style={{ marginBottom:16 }}>
        <div>
          ...
          <div className="ssub">{withCV.length} passed candidates with CVs</div>
        </div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
        {[
          ...
          { label:'With experience', value: sorted.filter(c => c.cvParsed?.totalExperience).length },
          { label:'With education', value: sorted.filter(c => c.cvParsed?.education?.length > 0).length },
          { label:'With skills', value: sorted.filter(c => c.cvParsed?.skills?.length > 0).length },
        ].map(s => (
          <div key={s.label} style={{ padding:'8px 14px', background:'var(--paper2)', borderRadius:8, fontSize:12 }}>
            <span style={{ color:'var(--ink3)' }}>{s.label}: </span><strong>{s.value}</strong>
          </div>
        ))}
      </div>
            ...
            return (
              <div key={c.id} className="card" style={{ padding:0, overflow:'hidden' }}>
                <div style={{ padding:'14px 20px', display:'flex', alignItems:'center', gap:14, cursor:'pointer' }}
                  ...
                  <div style={{ fontSize:13, fontWeight:700, color:'var(--ink3)', minWidth:28, textAlign:'center' }}>{idx+1}</div>
...
export default function Dashboard() {
  ...
  const [form, setForm]                   = useState({ title:'', dept:'', threshold:12, isMTO:false, ageLimit:28, recruiterEmail:'', difficulty:'medium', expiryDate:'' })
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      // Only show cognitive assessment roles and candidates (exclude leadership)
      const cogRoles = (r||[]).filter(role => role.assessmentType !== 'leadership')
      const cogCands = (c||[]).filter(cand => cand.assessmentType !== 'leadership')
      setRoles(cogRoles); setAllCandidates(cogCands)
      setOnline(true)
    } catch(e) {
  ...
  const makeRole = () => {
    ...
    dbAddRole({
      ...
      difficulty: form.difficulty || 'medium', archived: false, expiryDate: form.expiryDate || ''
    })
        ...
        {!globalSearch.trim() && (
          <>
            <div style={{ display:'flex', gap:0, marginBottom:20, background:'var(--paper2)', borderRadius:'var(--r-sm)', padding:3, border:'1px solid var(--line)', width:'fit-content' }}>
              {[['roles','📋 Roles'],['ranking','🏆 Ranking'],['analytics','📊 Analytics'],['cvinsights','🎓 CV Insights'],['intray','📝 In-Tray Scoring']].map(([id,label])=>(
                ...
                {roles.filter(r => showArchived ? r.archived : !r.archived).length === 0 ? (
                  ...
                  roles.filter(r => showArchived ? r.archived : !r.archived).map(role => (
                    <RoleCard
                      ...
                      onEditThreshold={(id,val)=>{ if(id.endsWith('_expiry')){dbUpdateRole(id.replace('_expiry',''),{expiryDate:val})}else{dbUpdateRole(id,{threshold:Number(val)})}; refresh()}}
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:13}}>
              ...
              <div className="fg">
                ...
                <span style={{fontSize:11,color:'var(--ink3)'}}>After this date candidates cannot access the link. Leave blank for no expiry.</span>
              </div>
      ...
      {expiryModal&&(
        <div className="overlay" onClick={()=>setExpiryModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="msub">Set, change, or remove the expiry date for this role link.</div>
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div className="fg">
                ...
                <span style={{fontSize:11,color:'var(--ink3)'}}>Candidates cannot access the link after this date.</span>
              </div>
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                <button className="btn btn-g btn-sm" style={{fontSize:11}}
                  onClick={()=>{const d=new Date();d.setDate(d.getDate()+7);setExpiryInput(d.toISOString().split('T')[0])}}>
                  +7 days
                </button>
                <button className="btn btn-g btn-sm" style={{fontSize:11}}
                  onClick={()=>{const d=new Date();d.setDate(d.getDate()+14);setExpiryInput(d.toISOString().split('T')[0])}}>
                  +14 days
                </button>
                <button className="btn btn-g btn-sm" style={{fontSize:11}}
                  onClick={()=>{const d=new Date();d.setDate(d.getDate()+30);setExpiryInput(d.toISOString().split('T')[0])}}>
                  +30 days
                </button>
                <button className="btn btn-g btn-sm" style={{fontSize:11}}
                  onClick={()=>{const d=new Date();d.setMonth(d.getMonth()+3);setExpiryInput(d.toISOString().split('T')[0])}}>
                  +3 months
                </button>
                <button className="btn btn-s btn-sm" style={{fontSize:11,color:'var(--bad)'}}
                  onClick={()=>setExpiryInput('')}>
                  Remove Expiry
                </button>
              </div>
              {expiryModal.expiryDate && (
                <div style={{fontSize:12,color:'var(--ink3)',padding:'10px 14px',background:'var(--paper2)',borderRadius:8}}>
                  Current expiry: <strong>{new Date(expiryModal.expiryDate).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</strong>
                </div>
              )}
            </div>
            <div style={{display:'flex',gap:10,marginTop:20}}>
              ...
              <button className="btn btn-p" style={{flex:1,justifyContent:'center'}} onClick={()=>{
                dbUpdateRole(expiryModal.id,{expiryDate:expiryInput})
                setExpiryModal(null)
                refresh()
              }}>
                {expiryInput ? 'Save Expiry Date' : 'Remove Expiry'}
              </button>
            </div>
          </div>
        </div>
