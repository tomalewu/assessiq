...
function exportResultsExcel(results) {
  const headers = [
    ...
    'Raw Total /100',
    'Drive & Initiative (1-5)','Drive Level',
    'Thinking Quality (1-5)','Thinking Level',
  ...
  html += 'th{background:#1e1b4b;color:#fff;padding:8px 10px;border:1px solid #ccc;text-align:left;white-space:nowrap}'
  html += 'td{padding:6px 10px;border:1px solid #ddd;vertical-align:top}'
  html += '.flag{background:#fee2e2;color:#991b1b;font-weight:bold}'
  html += '.top{background:#d1fae5}'
  ...
  sorted.forEach(function(r, idx) {
    ...
    var row = [
      ...
      r.rawTotal,
      r.competencies.driveInitiative.score, r.competencies.driveInitiative.level,
      r.competencies.thinkingQuality.score, r.competencies.thinkingQuality.level,
      r.competencies.communication.score, r.competencies.communication.level,
      r.overallAssessment,
      r.integrityFlag ? 'YES - ' + (r.integrityNote || '') : 'No',
      r.legibilityNotes || ''
    ]
  ...
  a.href = url; a.download = 'intray_scoring_results.xls'; a.click()
...
export default function InTrayScoring() {
  ...
  function handleFiles(incoming) {
    var valid = Array.from(incoming).filter(function(f) {
      return ACCEPTED.includes(f.type) || f.name.match(/\.(pdf|jpg|jpeg|png|webp)$/i)
    })
    setFiles(function(prev) {
      var names = prev.map(function(f) { return f.name })
      var newFiles = valid.filter(function(f) { return !names.includes(f.name) })
